...
int request_cs(const void * self) {
    ...
    lamport_time++;
    ctx->my_request = get_lamport_time();
...
void process_messages(struct context * ctx, int * reply_count, int * done_msg_count) {
    ...
    while (1) {
        ...
        switch (msg.s_header.s_type) {
            case CS_REQUEST:
                if (msg.s_header.s_local_time < ctx->my_request ||
                (msg.s_header.s_local_time == ctx->my_request && last_from < ctx->id)) {
                    lamport_time++;
                    set_message_header(&msg, CS_REPLY, get_lamport_time());
                    send(ctx, last_from, &msg);
                } else {
                    ctx->plan_to_send[last_from] = 1;
                }
                break;
...
//// Stop receiving messages and go to CS
        if (*reply_count == ctx->n - 2 || *done_msg_count == ctx->n - 2) {
            break;
        }
    }
}
